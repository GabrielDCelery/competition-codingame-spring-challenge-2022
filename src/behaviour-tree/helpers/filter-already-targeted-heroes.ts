...
export class FilterAlreadyTargetedHeroes extends LeafNode {
    protected _execute({
        gameState,
        chosenHeroCommands,
        localCache,
        ...
        try {
            const entityIDsToFilter = localCache.get<number[]>({ key: LocalCacheKey.TARGET_ENEMY_HERO_IDS });
            ...
            Object.values(chosenHeroCommands).forEach((chosenHeroCommand) => {
                if (
                    [
                        ...
                        CommandType.FARM,
                        CommandType.SPELL_CONTROL,
                        CommandType.SPELL_SHIELD,
                    ].includes(chosenHeroCommand.type)
                ) {
            ...
            localCache.set<number[]>({ key: LocalCacheKey.TARGET_ENEMY_HERO_IDS, value: filteredEntityIDs });

            return true;
        } catch (error_) {
            console.error(`FilterAlreadyTargetedHeroes`);
            throw new Error(`FilterAlreadyTargetedHeroes`);
        }
    }
}
